<template>
  ...
  const response = await fetch(`http://localhost:8001/api/admin/unverified?page=${page}`, { headers });
  ...
  const response = await fetch(`http://localhost:8001/api/admin/${id}/accept`, {
  method: 'PUT',
  headers,
